...
```{r min-wage-dag, echo=FALSE, warning=FALSE, message=FALSE, out.width="55%", fig.show='hold', fig.align='center', include=TRUE, eval=TRUE, results = FALSE}
...
$(A - B) - (C - D)$
- The $A - B$ tells us the change over time of Top Gun pilots. How did maneuvering change for Top Gun pilots?
- The $C - D$ tells give us an estimate of the counterfactual. If Top Gun pilots are similar to non-Top Gun pilots in their maneuvering trends then $C - D$ gives us an estimate of what would have happened without Top Gun training. 
...
### How We Measure the Impact of Waiting Periods on Firearm Suicides
...
### How We Measure the Impact of Waiting Periods on Firearm Suicides (cont.)
...
\begin{align*}
{\color{green}{Y_{cst}}} = &\alpha + \beta {\color{blue}{\text{Waiting}_c}} + \gamma {\color{yellow}{\text{Year}_t}} + \\
&\delta {\color{red}{(\text{Waiting}_c \times \text{Year}_t)}} + \varepsilon_{it}
\end{align*}
...
- $Y_{cst}$ is the number of firearm suicides per 100,000 people in county $c$ state $s$ in year $t$
- $\text{Waiting}_c$ is a variable that equals 1 if county $c$ has a waiting period in year $t$
- $\text{Year}_t$ is a variable that equals 1 in year $t$
